...
//**
//* Рекурсивно собирает список файлов с заданными расширениями.
//*
//* @param string|array $paths Путь или массив путей к директориям.
//* @param bool         $useShell Если true — использовать shell-команды вместо PHP-функций.
//* @return array Ключи — относительные пути (для отображения), значения — полные пути к файлам.
//*/
function getLists($paths, bool $useShell = false): array {
    global $allowedExtensions;
    $result = [];

    // Если передали строку — превращаем в массив.
    if (is_string($paths)) {
        $paths = [$paths];
    }

    foreach ($paths as $path) {
        // Shell-режим: ls через escapeshellarg.
        if ($useShell) {
            $resolvedPath = rtrim(shell_exec($path) ?: '');
            $raw = shell_exec("ls " . escapeshellarg($resolvedPath) . "/* 2>/dev/null");
            $files = $raw ? explode("\n", trim($raw)) : [];
            foreach ($files as $file) {
                if ($file && is_file($file) && !is_link($file)) {
                    $ext = pathinfo($file, PATHINFO_EXTENSION);
                    if (in_array($ext, $allowedExtensions, true)) {
                        // Ключ — полный путь, значение — полный путь
                        $result[$file] = $file;
                    }
                }
            }
        } else {
            // PHP-режим: рекурсивный обход через Standard PHP Library
            if (!is_dir($path)) {
                continue;
            }
            ...
            foreach ($iterator as $item) {
                if ($item->isFile()) {
                    ...
                    if (in_array($ext, $allowedExtensions, true)) {
                        // Ключ — относительный путь от $path
                        $relative = substr($filePath, strlen($path) + 1);
                        $result[$relative] = $filePath;
                    }
                }
            }
        }
